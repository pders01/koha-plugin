...
Plugins that store data in their own tables via raw DBI can't inherit `Koha::Object::Mixin::AdditionalFields` and end up reimplementing the load / save / delete / bulk-read quartet. This patch exposes the mixin's behaviour through a new helper class `Koha::AdditionalFields::DBI` (entry point: `Koha::AdditionalFields::DBI->for_table($tablename)`), so a plugin gets the same load / save / delete / bulk semantics without subclassing `Koha::Object`.
